...
def get_year_value(year_str):
    """
    Converts dynamic frontend strings (e.g., 'BSc - 1st Year' or 'Honours Degree')
    into an integer hierarchy for the matchmaking algorithm.
    """
    if not year_str:
        return 0

    # Lowercase the string to make it case-insensitive
    clean_str = str(year_str).lower()

    # Match the exact patterns generated by the frontend JS
    if "1st year" in clean_str:
        return 1
    if "2nd year" in clean_str:
        return 2
    if "3rd year" in clean_str:
        return 3
    if "4th year" in clean_str:
        return 4
    if "advanced diploma" in clean_str:
        return 4  # Equivalent to 4th year
    if "honours" in clean_str:
        return 5
    if "masters" in clean_str:
        return 6
    if "phd" in clean_str or "doctorate" in clean_str:
        return 7
